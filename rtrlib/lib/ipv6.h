 ...
 * @returns An ipv6_addr, where all bits that aren't in the specified range are
 *	    set to 0.
*/
ipv6_addr ipv6_get_bits(const ipv6_addr *val, const uint8_t first_bit,
			const uint8_t quantity);

/**
 * Converts the passed ipv6_addr to string representation
 * @param[in] ip ipv6_addr
 * @param[out] str Pointer to a string buffer must be at least INET6_ADDRSTRLEN
 *		   bytes long.
 * @result 0 on success
 * @result -1 on error
*/
int ipv6_addr_to_str(const ipv6_addr *ip_addr, char *b,
		     const unsigned int len);

/**
 * Converts the passed IPv6 address in string representation to an ipv6_addr
 * struct.
...
/**
 * @ingroup util_h[{
 * Converts the passed IPv6 address first_bit network byte order to host byte
 * order.
 * @param[in] src Pointer to a uint32_t array storing a ipv6 address in network
 *		  byte order.
 * @param[out] dest Pointer a uint32_t array that will be used to store the
 *		    ipv6 addr in host byte order.
 * }
*/
