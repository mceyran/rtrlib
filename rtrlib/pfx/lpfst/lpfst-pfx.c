...
static lpfst_node *pfx_table_get_root(const struct pfx_table *pfx_table,
				      const ip_version ver);
...
static data_elem *pfx_table_find_elem(const node_data *data,
				      const pfx_record *record,
				      unsigned int *index);
static bool pfx_table_elem_matches(node_data *data, const uint32_t asn,
				   const uint8_t prefix_len);
static void pfx_table_notify_clients(pfx_table *pfx_table,
				     const pfx_record *record,
				     const bool added);
static int pfx_table_remove_id(pfx_table *pfx_table, lpfst_node **root,
			       lpfst_node *node, const rtr_socket *socket,
			       const unsigned int level);
static int pfx_table_node2pfx_record(lpfst_node *node, pfx_record records[],
				     const unsigned int ary_len);
static void pfx_table_free_reason(pfx_record **reason,
				  unsigned int *reason_len);

void pfx_table_notify_clients(pfx_table *pfx_table, const pfx_record *record,
			      const bool added)
{
			...
			do {
				...
				for (unsigned int i = 0; i < data->len; i++) {
					pfx_record record = {
						data->ary[i].asn,
						(root->prefix),
						root->len, data->ary[i].max_len,
						data->ary[i].socket
					};
					pfx_table_notify_clients(pfx_table,
								 &record,
								 false);
				}
				rm_node = (lpfst_remove(root, &(root->prefix),
							root->len, 0));
				assert(rm_node != NULL);
...
data_elem *pfx_table_find_elem(const node_data *data,
			       const pfx_record *record, unsigned int *index)
{
	for (unsigned int i = 0; i < data->len; i++) {
		if (data->ary[i].asn == record->asn
		    && data->ary[i].max_len == record->max_len
...
inline lpfst_node *pfx_table_get_root(const struct pfx_table *pfx_table,
				      const ip_version ver)
{
...
{
	//if index is not the last elem in the list, move all other elems
...
int pfx_table_add(struct pfx_table *pfx_table, const pfx_record *record)
{
	...
	if (root != NULL) {
		bool found;
		lpfst_node *node =
		    lpfst_lookup_exact(root, &(record->prefix),
				       record->min_len, &lvl, &found);
		//node with prefix exists
		if (found) {
			...
			if (pfx_table_create_node(&new_node, record) ==
			    PFX_ERROR) {
				pthread_rwlock_unlock(&pfx_table->lock);
				return PFX_ERROR;
			}
...
int pfx_table_remove(struct pfx_table *pfx_table, const pfx_record *record)
{
	...
	bool found;
	lpfst_node *node =
	    lpfst_lookup_exact(root, &(record->prefix), record->min_len, &lvl,
			       &found);
	if (!found) {
		pthread_rwlock_unlock(&pfx_table->lock);
	...
	if (ndata->len == 0) {
		node =
		    lpfst_remove(node, &(record->prefix), record->min_len, lvl);
		assert(node != NULL);
...
bool pfx_table_elem_matches(node_data *data, const uint32_t asn,
			    const uint8_t prefix_len)
{
	for (unsigned int i = 0; i < data->len; i++) {
		if (data->ary[i].asn != 0 && data->ary[i].asn == asn
		    && prefix_len <= data->ary[i].max_len)
			return true;
	}
	return false;
}

int pfx_table_node2pfx_record(lpfst_node *node, pfx_record *records,
			      const unsigned int ary_len)
{
...
inline void pfx_table_free_reason(pfx_record **reason,
				  unsigned int *reason_len)
{
	if (reason != NULL) {
		free(*reason);
...
int pfx_table_validate_r(struct pfx_table *pfx_table, pfx_record **reason,
			 ...
			 const ip_addr *prefix, const uint8_t prefix_len,
			 pfxv_state *result)
{
	...
	if (reason_len != NULL && reason != NULL) {
		...
		*reason = realloc(*reason, *reason_len * sizeof(pfx_record));
		...
		if (pfx_table_node2pfx_record(node, *reason, *reason_len) ==
		    PFX_ERROR) {
			pthread_rwlock_unlock(&pfx_table->lock);
			pfx_table_free_reason(reason, reason_len);
			return PFX_ERROR;
	...
	while (!pfx_table_elem_matches(node->data, asn, prefix_len)) {
		//post-incr lvl, lpfst_lookup is performed on child_nodes =>
		//parent lvl + 1
		if (ip_addr_is_zero(ip_addr_get_bits(prefix, lvl++, 1)))
			node =
			    lpfst_lookup(node->lchild, prefix, prefix_len,
					 &lvl);
		else
			node =
			    lpfst_lookup(node->rchild, prefix, prefix_len,
					 &lvl);
		...
		if (reason_len != NULL && reason != NULL) {
			...
			*reason_len += ((node_data *) node->data)->len;
			*reason =
			    realloc(*reason, *reason_len * sizeof(pfx_record));
			...
			if (pfx_table_node2pfx_record
			    (node, start,
			     ((node_data *) node->data)->len) == PFX_ERROR) {
				pthread_rwlock_unlock(&pfx_table->lock);
				pfx_table_free_reason(reason, reason_len);
				return PFX_ERROR;
			}
...
int pfx_table_validate(struct pfx_table *pfx_table, const uint32_t asn,
		       ...
		       pfxv_state *result)
{
	return pfx_table_validate_r(pfx_table, NULL, NULL, asn, prefix,
				    prefix_len, result);
}
...
int pfx_table_src_remove(struct pfx_table *pfx_table, const rtr_socket *socket)
{
	for (unsigned int i = 0; i < 2; i++) {
		lpfst_node **root =
		    (i == 0 ? &(pfx_table->ipv4) : &(pfx_table->ipv6));
		pthread_rwlock_wrlock(&(pfx_table->lock));
		if (*root != NULL) {
			int rtval =
			    pfx_table_remove_id(pfx_table, root, *root, socket,
						0);
			if (rtval == PFX_ERROR)
				return PFX_ERROR;
		}
...
int pfx_table_remove_id(pfx_table *pfx_table, lpfst_node **root,
			lpfst_node *node, const rtr_socket *socket,
			const unsigned int level)
{
	assert(node != NULL);
	...
	//data from removed node are replaced from data from child nodes (if
	//children exists), same node must be checked again if it was replaced
	//with previous child node data
	while (check_node) {
		node_data *data = node->data;
		for (unsigned int i = 0; i < data->len; i++) {
			while (data->len > i && data->ary[i].socket == socket) {
				pfx_record record = {
					data->ary[i].asn, node->prefix,
					node->len,
					data->ary[i].max_len,
					data->ary[i].socket
				};
				if (pfx_table_del_elem(data, i) == PFX_ERROR)
					return PFX_ERROR;
				pfx_table_notify_clients(pfx_table, &record,
							 false);
			}
		}
		if (data->len == 0) {
			lpfst_node *rm_node =
			    lpfst_remove(node, &(node->prefix), node->len,
					 level);
			assert(rm_node != NULL);
	...
	if (node->lchild != NULL) {
		if (pfx_table_remove_id
		    (pfx_table, root, node->lchild, socket,
		     level + 1) == PFX_ERROR)
			return PFX_ERROR;
	}
	if (node->rchild != NULL)
		return pfx_table_remove_id(pfx_table, root, node->rchild,
					   socket, level + 1);
	return PFX_SUCCESS;
}

static void pfx_table_for_each_rec(lpfst_node *n,
