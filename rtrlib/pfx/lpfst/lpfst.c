...
void lpfst_insert(lpfst_node *root_node, lpfst_node *new_node,
			 ...
			 const uint8_t mask_len, unsigned int *level)
{
	while (root_node != NULL) {
		if (root_node->len <= mask_len
		    &&
		    ip_addr_equal(ip_addr_get_bits
				  (&(root_node->prefix), 0, root_node->len),
				  ip_addr_get_bits(prefix, 0, root_node->len)))
			       ...
			       bool *found)
{
	*found = false;
	while (root_node != NULL) {
		...
		if (root_node->len == mask_len
		    && ip_addr_equal(root_node->prefix, *prefix)) {
			*found = true;
			return (lpfst_node *) root_node;
		}
			 ...
			 const uint8_t mask_len, const unsigned int level)
{
	if (root_node->len == mask_len
		...
		if (lpfst_is_leaf(root_node)) {
			...
			if (root_node->lchild != NULL
				...
				|| (root_node->lchild->len >
				    root_node->rchild->len))) {
				...
				return lpfst_remove(root_node->lchild,
						    &(root_node->lchild->
						      prefix),
						    root_node->lchild->len,
						    level + 1);
				...
				return lpfst_remove(root_node->rchild,
						    &(root_node->rchild->
						      prefix),
						    root_node->rchild->len,
						    level + 1);
	...
	if (ip_addr_is_zero(ip_addr_get_bits(prefix, level, 1))) {
		if (root_node->lchild == NULL)
			return NULL;
		return lpfst_remove(root_node->lchild, prefix, mask_len,
				    level + 1);
	} else {
		if (root_node->rchild == NULL)
			return NULL;
		return lpfst_remove(root_node->rchild, prefix, mask_len,
				    level + 1);
	}
}
