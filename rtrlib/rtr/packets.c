...
static int rtr_receive_pdu(rtr_socket * rtr_socket, void *pdu,
			   const size_t pdu_len, const time_t timeout);
static int rtr_send_error_pdu(const rtr_socket * rtr_socket,
			      const void *erroneous_pdu, const uint32_t pdu_len,
			      const pdu_error_type error, const char *text,
...
static int rtr_send_pdu(const rtr_socket * rtr_socket, const void *pdu,
			const unsigned len);
static int rtr_update_pfx_table(rtr_socket * rtr_socket, const void *pdu);
static int rtr_set_last_update(rtr_socket * rtr_socket);
void rtr_prefix_pdu_2_pfx_record(const rtr_socket * rtr_socket, const void *pdu,
				 pfx_record * pfxr, const pdu_type type);
/*
 * @brief Appends the Prefix PDU pdu to ary.
 */
static int rtr_store_prefix_pdu(rtr_socket * rtr_socket, const void *pdu,
				const unsigned int pdu_size, void **ary,
			     ...
			     const rtr_socket_state new_state)
{
	...
	if (rtr_socket->connection_state_fp != NULL)
		rtr_socket->connection_state_fp(rtr_socket, new_state,
						rtr_socket->
 ...
 */
int rtr_receive_pdu(rtr_socket * rtr_socket, void *pdu, const size_t pdu_len,
		    const time_t timeout)
{
	...
	if (rtr_socket->state == RTR_SHUTDOWN)
		return RTR_ERROR;
	error =
	    tr_recv_all(rtr_socket->tr_socket, pdu, sizeof(pdu_header),
			timeout);
	if (error < 0)
		goto error;
	else
	...
	if (remaining_len > 0) {
		if (rtr_socket->state == RTR_SHUTDOWN)
			return RTR_ERROR;
		error =
		    tr_recv_all(rtr_socket->tr_socket,
				(((char *)pdu) + sizeof(pdu_header)),
	...
	if (header.type == IPV4_PREFIX || header.type == IPV6_PREFIX) {
		if (((pdu_ipv4 *) pdu)->zero != 0) {
			RTR_DBG1
			    ("Warning: Zero field of received Prefix PDU doesn't contain 0");
		}
	}
...
error:
	...
	if (error == -1) {
		...
		const char *txt =
		    "corrupt data received, length value in PDU is too small";
		rtr_send_error_pdu(rtr_socket, pdu, sizeof(pdu_header),
				   CORRUPT_DATA, txt, sizeof(txt));
	} else if (error == 4) {
		RTR_DBG1("PDU too big");
		char txt[42];
		snprintf(txt, sizeof(txt),
			 "PDU too big, max. PDU size is: %u bytes",
			 RTR_MAX_PDU_LEN);
		RTR_DBG("%s", txt);
		rtr_send_error_pdu(rtr_socket, pdu, sizeof(pdu_header),
				   CORRUPT_DATA, txt, sizeof(txt));
		...
		rtr_send_error_pdu(rtr_socket, pdu, header.len,
				   UNSUPPORTED_PDU_TYPE, NULL, 0);
	} else if (error == 16) {
		RTR_DBG1("PDU with unsupported Protocol version received");
		rtr_send_error_pdu(rtr_socket, pdu, header.len,
				   UNSUPPORTED_PROTOCOL_VER, NULL, 0);
	}
...
int rtr_store_prefix_pdu(rtr_socket * rtr_socket, const void *pdu,
			 const unsigned int pdu_size, void **ary,
			 unsigned int *ind, unsigned int *size)
{
	...
	if ((*ind) >= *size) {
		*size += 100;
		...
		if (tmp == NULL) {
			...
			RTR_DBG("%s", txt);
			rtr_send_error_pdu(rtr_socket, pdu, pdu_size,
					   INTERNAL_ERROR, txt, sizeof(txt));
			rtr_change_socket_state(rtr_socket, RTR_ERROR_FATAL);
			free(ary);
			ary = NULL;
			return RTR_ERROR;
...
int rtr_sync(rtr_socket * rtr_socket)
{
	...
	int rtval =
	    rtr_receive_pdu(rtr_socket, pdu, RTR_MAX_PDU_LEN, RTR_RECV_TIMEOUT);
	...
	while (type == SERIAL_NOTIFY) {
		...
		rtval =
		    rtr_receive_pdu(rtr_socket, pdu, RTR_MAX_PDU_LEN,
				    RTR_RECV_TIMEOUT);
		if (rtval == TR_WOULDBLOCK) {
	...
	if (type == CACHE_RESPONSE) {
		...
		if (rtr_socket->request_session_id) {
			if (rtr_socket->last_update != 0) {
				...
				pfx_table_src_remove(rtr_socket->pfx_table,
						     rtr_socket);
			...
			if (rtr_socket->session_id != cr_pdu->reserved) {
				...
				rtr_send_error_pdu(rtr_socket, NULL, 0,
						   CORRUPT_DATA, txt,
						   sizeof(txt));
				rtr_change_socket_state(rtr_socket,
							RTR_ERROR_FATAL);
		...
		RTR_DBG
		    ("Expected Cache Response PDU but received PDU Type (Type: %u)",
		     ((pdu_header *) pdu)->type);
		const char *txt =
		    "Unexpected PDU received in data synchronisation";
		rtr_send_error_pdu(rtr_socket, pdu, sizeof(pdu_header),
				   CORRUPT_DATA, txt, sizeof(txt));
		return RTR_ERROR;
	}
	...
	do {
		...
		type = rtr_get_pdu_type(pdu);
		if (type == IPV4_PREFIX) {
			if (rtr_store_prefix_pdu
			    (rtr_socket, pdu, sizeof(pdu_ipv4),
			     (void **)&ipv4_pdus, &ipv4_pdus_nindex,
			     &ipv4_pdus_size) == RTR_ERROR)
				return RTR_ERROR;
		} else if (type == IPV6_PREFIX) {
			if (rtr_store_prefix_pdu
			    (rtr_socket, pdu, sizeof(pdu_ipv6),
			     (void **)&ipv6_pdus, &ipv6_pdus_nindex,
			     &ipv6_pdus_size) == RTR_ERROR)
				return RTR_ERROR;
		} else if (type == EOD) {
			...
			if (eod_pdu->session_id != rtr_socket->session_id) {
				...
				snprintf(txt, sizeof(txt),
					 ...
					 rtr_socket->session_id,
					 eod_pdu->session_id);
				rtr_send_error_pdu(rtr_socket, pdu,
						   RTR_MAX_PDU_LEN,
						   CORRUPT_DATA, txt,
						   strlen(txt) + 1);
				rtr_change_socket_state(rtr_socket,
			...
			for (unsigned int i = 0; i < ipv4_pdus_nindex; i++) {
				if (rtr_update_pfx_table
				    (rtr_socket,
				     &(ipv4_pdus[i])) == PFX_ERROR) {
					//undo all record updates, except the last which produced the error
					RTR_DBG
					    ("Error during data synchronisation, recovering Serial Nr. %u state",
					     rtr_socket->serial_number);
					for (unsigned int j = 0;
					     (j < i) && (rtval == RTR_SUCCESS);
					     j++)
						rtval =
						    rtr_undo_update_pfx_table
						    (rtr_socket,
						     &(ipv4_pdus[j]));
					if (rtval == RTR_ERROR) {
						RTR_DBG1
						...
						pfx_table_src_remove
						    (rtr_socket->pfx_table,
						     rtr_socket);
						rtr_socket->request_session_id =
						    true;
					}
			...
			for (unsigned int i = 0; i < ipv6_pdus_nindex; i++) {
				if (rtr_update_pfx_table
				    (rtr_socket,
				     &(ipv6_pdus[i])) == PFX_ERROR) {
					//undo all record updates if error occured
					RTR_DBG
					    ("Error during data synchronisation, recovering Serial Nr. %u state",
					     rtr_socket->serial_number);
					for (unsigned int j = 0;
					     j < ipv4_pdus_nindex; j++)
						rtval =
						    rtr_undo_update_pfx_table
						    (rtr_socket,
						     &(ipv4_pdus[j]));
					for (unsigned int j = 0;
					     (j < i) && (rtval == PFX_SUCCESS);
					     j++)
						rtval =
						    rtr_undo_update_pfx_table
						    (rtr_socket,
			...
			RTR_DBG("Received unexpected PDU (Type: %u)",
				((pdu_header *) pdu)->type);
			const char *txt =
			    "Unexpected PDU received during data synchronisation";
			rtr_send_error_pdu(rtr_socket, pdu, sizeof(pdu_header),
					   CORRUPT_DATA, txt, sizeof(txt));
			free(ipv4_pdus);
			free(ipv6_pdus);
	...
	if (rtr_set_last_update(rtr_socket) == RTR_ERROR)
		return RTR_ERROR;
	RTR_DBG
	    ("Sync successfull, received %u Prefix PDUs, session_id: %u, SN: %u",
	     (ipv4_pdus_nindex + ipv6_pdus_nindex), rtr_socket->session_id,
	     rtr_socket->serial_number);
	return RTR_SUCCESS;
}
				 ...
				 pfx_record * pfxr, const pdu_type type)
{
	...
	if (type == IPV4_PREFIX) {
		...
		memcpy(pfxr->prefix.u.addr6.addr, ipv6->prefix,
		       sizeof(pfxr->prefix.u.addr6.addr));
...
int rtr_update_pfx_table(rtr_socket * rtr_socket, const void *pdu)
{
	...
	pfx_record pfxr;
	size_t pdu_size =
	    (type == IPV4_PREFIX ? sizeof(pdu_ipv4) : sizeof(pdu_ipv6));
	rtr_prefix_pdu_2_pfx_record(rtr_socket, pdu, &pfxr, type);
	...
	if (((pdu_ipv4 *) pdu)->flags == 1)
		...
		const char *txt =
		    "Prefix PDU with invalid flags value received";
		RTR_DBG("%s", txt);
		rtr_send_error_pdu(rtr_socket, pdu, pdu_size, CORRUPT_DATA, txt,
				   sizeof(txt));
		return RTR_ERROR;
	}
	...
	if (rtval == PFX_DUPLICATE_RECORD) {
		...
		RTR_DBG
		    ("Duplicate Announcement for record: %s/%u-%u, ASN: %u, received",
		     ip, pfxr.min_len, pfxr.max_len, pfxr.asn);
		rtr_send_error_pdu(rtr_socket, pdu, pdu_size,
				   DUPLICATE_ANNOUNCEMENT, NULL, 0);
		rtr_change_socket_state(rtr_socket, RTR_ERROR_FATAL);
		return RTR_ERROR;
	} else if (rtval == PFX_RECORD_NOT_FOUND) {
		RTR_DBG1("Withdrawal of unknown record");
		rtr_send_error_pdu(rtr_socket, pdu, pdu_size,
				   WITHDRAWAL_OF_UNKNOWN_RECORD, NULL, 0);
		rtr_change_socket_state(rtr_socket, RTR_ERROR_FATAL);
		return RTR_ERROR;
	} else if (rtval == PFX_ERROR) {
		const char *txt = "PFX_TABLE Error";
		RTR_DBG("%s", txt);
		rtr_send_error_pdu(rtr_socket, pdu, pdu_size, INTERNAL_ERROR,
				   txt, sizeof(txt));
		rtr_change_socket_state(rtr_socket, RTR_ERROR_FATAL);
		return RTR_ERROR;
	}
...
int rtr_wait_for_sync(rtr_socket * rtr_socket)
{
	...
	time_t wait =
	    (rtr_socket->last_update + rtr_socket->polling_period) - cur_time;
	if (wait < 0)
		wait = 0;
...
int rtr_send_error_pdu(const rtr_socket * rtr_socket, const void *erroneous_pdu,
		       const uint32_t pdu_len, const pdu_error_type error,
		       const char *text, const uint32_t text_len)
{
...
int rtr_send_pdu(const rtr_socket * rtr_socket, const void *pdu,
		 const unsigned len)
{
	...
	if (rtr_socket->state == RTR_SHUTDOWN)
		return RTR_ERROR;
	const int rtval =
	    tr_send_all(rtr_socket->tr_socket, pdu_converted, len,
			RTR_SEND_TIMEOUT);
...
int rtr_handle_error_pdu(rtr_socket * rtr_socket, const void *buf)
{
	...
	case UNSUPPORTED_PDU_TYPE:
		...
		break;
	default:
		RTR_DBG("error unknown, server sent unsupported error code %u",
			pdu->error_code);
		rtr_change_socket_state(rtr_socket, RTR_ERROR_FATAL);
		break;
	}

	const uint32_t len_err_txt =
	    ntohl(*((uint32_t *) ((pdu->rest) + pdu->len_enc_pdu)));
	if (len_err_txt > 0) {
		if ((sizeof(pdu->ver) + sizeof(pdu->type) +
		     sizeof(pdu->error_code) + sizeof(pdu->len) +
		     sizeof(pdu->len_enc_pdu) + pdu->len_enc_pdu + 4 +
		     len_err_txt) != pdu->len)
			RTR_DBG1
			    ("error: Length of error text contains an incorrect value");
		else {
			//assure that the error text contains an terminating \0 char
			char txt[len_err_txt + 1];
			char *pdu_txt =
			    (char *)pdu->rest + pdu->len_enc_pdu + 4;
			snprintf(txt, len_err_txt + 1, "%s", pdu_txt);
			RTR_DBG
			    ("Error PDU included the following error msg: \'%s\'",
			     txt);
		}
	}
...
{
	if (rtr_send_pdu(rtr_socket, &pdu_reset_query, sizeof(pdu_reset_query))
	    != RTR_SUCCESS) {
		rtr_change_socket_state(rtr_socket, RTR_ERROR_TRANSPORT);
		return RTR_ERROR;
	}
