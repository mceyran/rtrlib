...
void rtr_init(rtr_socket * rtr_socket, tr_socket * tr,
	      ...
	      void *fp_param)
{
	...
	if (polling_period == 0)
		...
		rtr_socket->polling_period =
		    (polling_period >
		     (3600 - RTR_RECV_TIMEOUT) ? (3600 -
						  RTR_RECV_TIMEOUT) :
		     polling_period);
	rtr_socket->cache_timeout =
	    (cache_timeout ==
	     0 ? (rtr_socket->polling_period / 2) : cache_timeout);
...
{
	int rtval =
	    pthread_create(&(rtr_socket->thread_id), NULL,
			   (void *(*)(void *))&rtr_fsm_start, rtr_socket);
	if (rtval == 0)
		return RTR_SUCCESS;
	return RTR_ERROR;
}
...
void rtr_purge_outdated_records(rtr_socket * rtr_socket)
{
	...
	if (rtval == -1
	    || (rtr_socket->last_update + rtr_socket->cache_timeout) >
	    cur_time) {
		if (rtval == -1)
	...
	while (1) {
		if (rtr_socket->state == RTR_CONNECTING) {
			...
			if (tr_open(rtr_socket->tr_socket) == TR_ERROR) {
				...
				if (rtr_send_serial_query(rtr_socket) ==
				    RTR_SUCCESS)
					rtr_change_socket_state(rtr_socket,
								RTR_SYNC);
				else
